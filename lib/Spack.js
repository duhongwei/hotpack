...
export default class extends SpackEvent {
  constructor({ config }) {
    ...
    this.keyFromWebPathConfig = config.keyFromWebPath
  ...
  getKeyFromWebPath({ path, file }) {
    ...
    for (let item of this.keyFromWebPathConfig) {
  ...
  addKeyFromWebPathConfig({ test, resolve }) {
    if (!test || !resolve) {
      throw new Error('test and resolve required!')
    }
    this.keyFromWebPathConfig.push({
      test,
      resolve
    })
  }
