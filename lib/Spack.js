...
export default class extends SpackEvent {
  ...
  async init() {
    ...
    for (let item of plugin) {
      ...
      if (typeof item == 'string') {
        key = item
        let p = `../plugin/${item}.js`
        p = path.join(__dirname, p)
        let pluginObj = await import(util.getImportUrl(p))
  ...
  getKeyFromPhysicalPath(path) {
    if (!path) throw new Error('path required')
    let key = path.replace(this.config.src, '').split(path.sep).join('/')
    if (key.startsWith('/')) {
      key = key.substr(1)
    }
  ...
  getKeyFromWebPath({ fileKey, webPath }) {
    ...
    if (!webPath.includes('.')) {
      ...
      if (this.fs.existsSync(path1)) {
        ...
        throw new Error(`both ${path1} and ${path2} not exist,fileKey is ${fileKey},webPath is ${webPath}`)
      }
    }
