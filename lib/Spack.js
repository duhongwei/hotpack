...
export default class extends SpackEvent {
  ...
  async init() {
    ...
    for (let item of plugin) {
      ...
      if (key == 'event') {
        this.config.logger.log(`init plugin ${key} | ${opt.name}`)
      }
      else {
        this.config.logger.log(`init plugin ${key}`)
      }
      opt = opt || {}
  ...
  async run() {
    ...
    for (const { test, key, plugin, name } of this.plugin) {

      if (typeof plugin === 'function') {
        this.isPro() && cosnole.log()
        if (key === 'event') {
          this.config.logger.log(`run plugin ${key} | ${name}`)
        }
        else {

          this.config.logger.log(`run plugin ${key}`)
        }
